...
- **해결 (Solution)**: 모든 API에 DB 조회를 넣는 대신, 결제 정합성이 중요한 Order, Address 서비스에만 'DB 실시간 재검증(validateUserRoleFromDB)' 가드 로직을 추가함. 토큰의 유효성과 별개로 DB 상의 실제 유저 상태와 권한 일치 여부를 매 요청마다 교차 검증하는 하이브리드 인가(Defense in Depth) 구조 설계.
